# MentalMapping アーキテクチャ設計書 v1.2.3
...
| 12 | daily_aspects | str | 自動 | L | **実装先行で Phase 1 時点から自動生成・充填済み**。既存 daily_aspects の形式は `[T] {transit} → [N] {natal} {aspect} ({orb}°) [{type}]` の改行区切り文字列 |
...
- daily_aspects 列は **実装先行で Phase 1 時点から自動生成済み**（v1.2.2 で実態に合わせて記述削除。形式詳細は §4.1 L 列備考参照）
...
| v1.2.2 | 2026-04-19 | α-C Phase 2 マイグレーション完了（commit `95a84fd`）後、実データと運用実態に追従させる改訂。主要変更: (1) §A.5.3 増田確定表を全面書き直し。実データに存在しない 2 行（04-16 20:34 evening / 04-17 11:18 morning の訂正試行）を削除し、実データに存在する 3 行（row 11: 04-15 T12:37:23 evening retroactive・row 12: 04-16 T9:34:37 morning 手動追加・row 15: 04-17 T21:18:57 evening 訂正済）を追加。(2) §4.1 L 列備考を「Phase 2 で自動生成・Phase 1 は空文字」→「**実装先行で Phase 1 時点から自動生成・充填済み**。形式は `[T] {transit} → [N] {natal} {aspect} ({orb}°) [{type}]` の改行区切り文字列」に修正。同様に §5 Phase 2 申し送り事項からも daily_aspects 関連記述を削除。(3) §A.5.6 「masuda 2026-04-16 morning → not_recorded 生成」の確定対象を取り消し（実データに row 12 が存在するため対象外）。経緯を mm_notes #1 として残置。(4) §A.3 record_id 生成規則に**再生成ルール**を追記。recorded_at 訂正時は record_id の unix_ts も同期再生成する責務を明文化。将来 v1.3 以降で UUID 等への見直し候補を申し送り（mm_notes #2 参照）。(5) Sheets 直接編集の経緯類型として**パターン 2a/2b/2c/2d** を §A.5.3 に新設。Claude.ai CSV 読み違い事例（v5.0 notes 連動）も併記。(6) §A.7.6 に α-D GAS バッチ起動時の追加申し送り（mm_notes #7 参照義務）を追記。(7) **mm_notes #2〜#7 を採番起票**: #2 (masuda row 15 入力ミスと事後訂正) / #3 (masuda row 11 入力ミス未訂正・西出放置判断) / #4 (コード先行・設計書追従パターンの窓割り当て指針) / #5 (Sheets 直接編集の運用観察・パターン統合) / #6 (訂正ダイアログ設計の事後検証事例) / #7 (α-D GAS バッチ起動時の仕様申し送り)。(8) **CLAUDE.md §9 新設**: 「Sheets 直接編集時の同期義務」運用ルール。Sheets 直接編集 3 同時実施（mm_notes 起票・設計書同期・Sprint α 通知）を必須化。本ルールの契機は v1.2 Sprint で発生したパターン 2a/2b/2c/2d 事例。 (9) **§A.6.1 入力ウィジェット仕様明確化**: time_of_day はデフォルト未選択 (`index=None`) + 送信ボタン disabled、4 指標 (mood/energy/thinking/focus) はデフォルト 3 (普通) 選択 (`index=2`)。設計意図を「time_of_day=必須明示選択 / 4 指標=デフォルト 3 で入力コスト軽減」として §6.4 主観状態記録の原則と整合させる。(10) **§A.6.2 input_user 選択 UI 明確化**: `settings.yaml` の `users` キーに存在する全ユーザーを表示（`realtime_window` 未定義ユーザーも除外しない）。具体例として suyasu（休眠ユーザー・realtime_window 未定義）がセレクタに表示され、記録時は entry_mode=pending が付与されることを明記。(11) **`config/settings.yaml` に suyasu セクション復活**: `display_name: "須安麗子"` / `sheet_name: "mood_log_suyasu"` を復元。`realtime_window` は意図的に未定義のまま（pending 付与対象）。Python コード変更なし（v1.2.1 で pending ロジック先行実装済み・`modules/entry_mode.py`）。 |
| v1.2.3 | 2026-04-19 | α-D Phase 1 GAS バッチ調査で発覚した UX 改善: `@st.dialog` 訂正ダイアログ（§A.6.3）が**既存 active = not_recorded** の場合に違和感（「記録があります」の文言が未入力観測に対して不適切、「試行として残す」が意味不成立）を解消。実装変更: (1) 既存 active レコードの `entry_mode` で文言・選択肢を出し分け。(2) **not_recorded 既存時は「未入力レコードへの実値記録」タイトル + 「実際の値で記録しますか?」文言 + 「上書き / キャンセル」2 択** に縮退。(3) 通常訂正（realtime/retroactive/pending 既存）は現行 3 択維持。(4) 上書き時のチェーン操作は両ケースで `find_active_record` + `supersede_active` を使用（**専用分岐コード不要**・`modules/record_chain` は predicate 1 関数追加のみ）。設計書 §A.6.3 を分岐表形式で書き直し、設計判断（not_recorded で試行を非表示にする理由）を明文化。新規 predicate `is_not_recorded_overwrite` を `modules/record_chain.py` に追加（Streamlit 非依存）。`tests/test_dialog_not_recorded.py` で predicate × チェーン動作 × 共通実装保証の 10 件テストを追加（合計 146 passed）。CLAUDE.md 更新なし（運用ルール変更なし）。 |
...
- `recorded_at` が訂正された場合、`record_id` の `unix_ts` 部分も**再生成して同期**する
- 契機:
  1. アプリ層の訂正操作時（§4.4 鎖更新フロー実行時）
  2. マイグレーションバッチ実行時（§A.5.7 ステップ 4 が `recorded_at` を読み取って付与）
- `record_id` を不変キーとして他テーブルが参照する設計を前提としてはならない（陳腐化リスク）
- **将来 v1.3 以降での見直し候補**: UUID 採用 / `recorded_at` 非依存形式 / 別途不変 `event_id` 列追加 等。
  陳腐化が業務影響を持つ前に検討する。`record_id` 陳腐化の具体事例は **mm_notes #2** 参照
...
#### A.5.3 増田の確定済みレコード（v1.2.2 で実データに追従）

以下は α-C Phase 2 完了時点（commit `95a84fd`）の実データに対応する。
α-C はマイグレーション実行時に本表のとおり付与済み。後続作業者は本表を実データ snapshot として参照する。
...
| 2026-04-15 | T12:37:23 | evening | **active** | null | **retroactive** | row 11。time_of_day を `evening` で記録したが本来 morning とされた入力ミス（パターン 2c）。西出は事後訂正せず観測値として保持判断 → **mm_notes #3** 参照 |
| 2026-04-16 | T9:34:37 | morning | **active** | null | realtime | row 12。Sheets 直接編集による手動追加（パターン 2b）。増田申告で西出が遡及登録。recorded_at が単桁時刻 ISO 非準拠で、`migrate_v1_2._normalize_iso_jst` で lenient 化済 → **mm_notes #1** 参照 |
| 2026-04-16 | 20:45 | evening | **active** | null | realtime | 04-16 evening の現行採用（直後の訂正試行は v1.2.2 で削除確定・パターン 2a） |
| 2026-04-17 | 07:45 | morning | **active** | null | realtime | 04-17 morning の現行採用（同日 11:18 の訂正試行は v1.2.2 で削除確定・パターン 2a） |
| 2026-04-17 | T21:18:57 | evening | **active** | null | realtime | row 15。Sheets 直接編集による事後訂正済（パターン 2c→2d）。`record_id` の `unix_ts` 部分は recorded_at 訂正に追従して再生成（§A.3 再生成ルール）→ **mm_notes #2** 参照 |

**v1.2.2 削除事項**:
- 旧 v1.2.1 表に記載されていた以下 2 件は実データに存在しないため**表から削除**:
  - 旧: 2026-04-16 20:34 evening (superseded → 20:45 chain)
  - 旧: 2026-04-17 11:18 morning (superseded, superseded_by=null 末端)
- 削除理由: 西出が当該訂正試行レコードを Sheets から物理削除した（パターン 2a）。
  鎖構造の末端解釈は仕様としては §4.4 / §A.4 に維持（将来同種事例発生時に有効）

**追加レコードの経緯類型（パターン番号は mm_notes #5 で統合定義）**:

| パターン | 概要 | 該当レコード |
|---------|------|------------|
| 1 | アプリ経由の通常記録（タイポ等の品質問題は Sheets 直接編集に起因） | 大半の既存レコード |
| 2a | **訂正試行レコードの直接削除** | 04-16 20:34 evening / 04-17 11:18 morning（v1.2.2 で表から削除） |
| 2b | **記録漏れの遡及追加**（Sheets 直接編集） | row 12: 04-16 T9:34:37 morning |
| 2c | **time_of_day 入力ミス**（Sheets 直接編集） | row 11: 04-15 T12:37 evening / row 15: 04-17 T11:18 evening の訂正前 |
| 2d | **入力ミスの事後訂正**（Sheets 直接編集） | row 15: 04-17 T21:18:57 evening の訂正後（2c→2d 連鎖） |

**Claude.ai CSV 読み違い事例**: 西出が Claude.ai に投入した CSV を Claude.ai が誤解釈し、
「row 11 が削除済」と推定した経緯あり。実データ確認で誤認判明（v5.0 notes 連動・**mm_notes #5** で詳細）。

**一般ルール**:
- 上記表に**ない**増田レコードは、α-C / 後続バッチが日付順スキャンし
  同一 `(date, time_of_day)` 複数行のみ recorded_at 時系列と西出確認に基づき active / superseded 判定する
- 本表の特殊事例（**04-17 morning の 07:45 active / 旧 11:18 末端**は v1.2.2 で削除済）は §4.4
  「active を決めるのはユーザー判断」に基づく確定事項。後続バッチは一般ルールではなく本表を正とする
- パターン 2a/2b/2c/2d の Sheets 直接編集は **新ルール「Sheets 直接編集時の同期義務」**（CLAUDE.md §9）の
  契機事例。今後同種編集時は mm_notes 起票・設計書同期・Sprint α 通知の 3 点同時実施が必須
...
**v1.2.2 削除事項と経緯**:
- 旧 v1.2.1 では「masuda 2026-04-16 morning → not_recorded を生成」と確定していたが、
  実データに row 12（`2026-04-16T9:34:37` morning）が存在することが α-C Phase 2 着手時に判明
- 経緯: 増田本人の申告により、西出が Sheets に手動追加（パターン 2b・**mm_notes #1** 参照）
- α-C Phase 2 実装側は `NOT_RECORDED_EXCLUDE = {("masuda", "2026-04-16", "morning")}` として
  ハードコード除外済（`devtools/migrate_v1_2_steps_populate.py`）
- 当該日付に active レコードが既に存在するため、§4.3.2 冪等性により not_recorded は**生成されない**

**現行（v1.2.2 以降）の運用**:
- α-C / 後続バッチは起点日 〜 実行日の範囲を走査
- 同一 `(input_user, date, time_of_day)` に**任意の record_status**のレコードが存在しない場合**のみ**
  not_recorded を生成する（§4.3.2 冪等性）
- v1.2.2 時点で既知の遡及生成対象は**ゼロ**（実データ手動追加により）
...
**4 指標ラジオボタン (mood / energy / thinking / focus)**:
- ウィジェット: `st.radio`（横並び `horizontal=True`）
- 選択肢: `[1, 2, 3, 4, 5]`
- **デフォルト 3（普通）選択**（`index=2`）
- ユーザーが変更しなければ「普通」値で送信される
- 設計意図:
  - **time_of_day** は記録の時間帯分類として**必須明示選択**（後続分析の主軸となるため）
  - **4 指標** は日次記録の入力コストを下げるため**デフォルト 3** とする。
    「記録した」事実が重要であり、値の精密さは二次的（§6.4 主観状態記録の原則）

**任意項目（sleep_hours / weather / medication / period）**:
- いずれもデフォルト値あり（睡眠 7.0 時間 / weather=`晴` / medication=False / period=False）
- 入力スキップ可能

#### A.6.2 input_user 選択 UI（v1.2.2 で明確化）

**基本仕様**:
- 毎回必須選択（毎回明示）
- サイドバーで現在の input_user を強調表示（`st.success` バッジ）
- worksheet 所有者と乖離する選択がされた場合、§4.6.2 (A) のリアルタイム警告を発動

**記録者セレクタの表示対象（v1.2.2 確定）**:
- `settings.yaml` の `users` キーに**存在する全ユーザーを表示**する
- `realtime_window` 未定義のユーザーも表示対象（除外しない）
- 表示順は `users` キーの定義順（masuda → nishide → suyasu）
- `display_name` が定義されていればそれを表示、未定義ならキー名を表示

**realtime_window 未定義ユーザーの扱い**:
- セレクタで選択可能（除外しない）
- 記録時は `entry_mode=pending` が自動付与される（§4.3.4）
- pending レコードは将来 realtime_window 確定時に再判定される
  （`devtools/rebalance_pending.py` 未実装・mm_notes #7 参照）

**具体例（v1.2.2 時点）**:
- `suyasu` は `realtime_window` 未定義の休眠ユーザーだが、
  `settings.yaml` に登録されているためセレクタに表示される
- suyasu が記録した場合、entry_mode は morning / evening いずれも `pending`

#### A.6.3 訂正ダイアログ（v1.2.3 で entry_mode 別分岐を追加）

同一 `(input_user, date, time_of_day)` に既存 active がある状態で記録ボタンを押下した
場合、ダイアログを発動する。**既存 active の `entry_mode` 値に応じて文言と選択肢を
出し分ける**。

##### (A) 通常訂正ダイアログ（既存 entry_mode が realtime / retroactive / pending）

- タイトル: 「既存記録の訂正」
- 文言: 「既に {date} {time_of_day_label} の記録があります。どうしますか?」
- 選択肢（**3 択**）:
  - **上書き**: §4.4 鎖構築フロー実行（旧 active を superseded に、新を active に）
  - **試行として残す**: 新レコードを `record_status=superseded, superseded_by=null` で追記
    （採用されなかった訂正試行として保存・§4.4 末端解釈）
  - **キャンセル**: 何もしない（pending_entry は次の記録ボタンまで保持）

##### (B) 未入力レコードへの実値記録ダイアログ（既存 entry_mode が not_recorded）

- タイトル: 「未入力レコードへの実値記録」
- 文言: 「**{date} {time_of_day_label}** は未入力レコード（`entry_mode=not_recorded`）として
  自動生成されています。実際の値で記録しますか?」
- 選択肢（**2 択**）:
  - **上書き**: 既存 not_recorded を superseded 化、新レコードを active で追記
    （§4.4.1 not_recorded → retroactive 昇格）
  - **キャンセル**: 何もしない

##### 設計判断: not_recorded で「試行として残す」を非表示にする理由

- not_recorded は「ユーザーが記録しなかった事実」を観測値として残したもの（§6.5）
- これに対して「試行として残す」（superseded_by=null 末端）を許可すると、
  「未入力に対する訂正試行」という**意味不成立**な状態が発生する
- 通常訂正は「ユーザー入力 vs ユーザー入力」の選択だが、not_recorded への上書きは
  「自動生成 vs ユーザー入力」の関係であり、ユーザー入力が一意に正となる
- 鎖構造の不変条件（active 1 件以内・線形リスト）は両ケースで同一

##### 分岐表（実装サマリー）

| 既存 entry_mode | ダイアログタイトル | 選択肢 | 上書き時のチェーン操作 |
|----------------|------------------|--------|----------------------|
| `realtime` / `retroactive` / `pending` | 既存記録の訂正 | 上書き / 試行として残す / キャンセル | §4.4 鎖構築 |
| `not_recorded` | 未入力レコードへの実値記録 | 上書き / キャンセル | §4.4 鎖構築 (= §4.4.1 昇格) |

##### 実装責務分離

- **分岐判定**: `app.py` が `is_not_recorded_overwrite()` 経由で UI 出し分け
- **チェーン操作**: `modules/record_chain.py` の `find_active_record` / `supersede_active`
  （両ケースで**同一実装**を使用。not_recorded 専用分岐コードは置かない）
- `find_active_record` は entry_mode を区別せず active 全件を対象とする（現行仕様維持）
- `is_not_recorded_overwrite()` は `modules/record_chain.py` に純関数 predicate として配置
  （Streamlit 非依存・テスト容易）
...
**α-D 起動時の追加申し送り**（v1.2.2 追記）:
- v1.2 / v1.2.1 / v1.2.2 改訂で生じた仕様前提の整理事項は **mm_notes #7** に集約。
  α-D は GAS バッチ実装着手前に **mm_notes #7** を必ず確認すること
  （pending ユーザー扱い・recorded_at 単桁時刻の lenient 化方針・record_id 再生成の責務境界 等）
